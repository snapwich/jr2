# The Harness is j2's own server: flue retires, the wire stays

j2 has been programming around its Harness, not with it. The evidence is the ADR trail itself.
[ADR-0018](0018-instance-agents-are-definitions-j2-assembles-the-harness.md)'s entire codegen-at-pod-start mechanism —
...
forced by a foreign advisory session j2 never asked for, and records a rejection ("there is nothing on the wire to
...
Meanwhile the thing j2 actually needs from flue is small, and j2 already owns its specification. The Orchestrator
...
- **j2 owns its Harness: `@j2/harness` at `packages/harness`**, built on `@earendil-works/pi-agent-core` +
...
- **The re-owned semantics are stated, not inherited:**
  - **Per-instance queue: accept and queue.** A Submission is promoted when it is the first unsettled Submission of its
    ...
    — flue queues, and the ordered abort depends on it — is now a design j2 asserts rather than a behavior it
    discovered.
  ...
  - **[ADR-0013](0013-adapter-hosts-the-agent-mcp-surface.md)'s enabling fact is reproduced as explicit code.** Each
    Submission connects a fresh MCP client to `$J2_ADAPTER_URL/mcp/<iid>` and lists tools, so the menu is per-turn and
    the Adapter still needs no push channel, no turn index, no second port. Menu tools keep the `mcp__j2__<name>` naming
    — shipped instructions and the printer's prefix-stripping depend on it. The previous turn's connection is closed
    deterministically; the retire-the-previous idiom and ADR-0023's 404-chase existed to work around flue API gaps that
    are gone.
...
- **Wait for flue 2.0.** Rejected: unreleased, unscheduled, and a rewrite of the surfaces j2 generates against
...
- **Fork flue at beta.9.** Rejected: ~17K lines of built output plus a CLI codegen owned to keep seven imported symbols,
  ...
  that left. And its durability layer, the one genuinely load-bearing-looking piece, is in-memory as j2 deploys it.
...
- **Kept verbatim**: the wire, the stub as the mechanics fixture, `AgentRunPort` and the admission ledger, ADR-0024's
  rule, receipt, and ordering, and the Adapter container split — restated in j2 terms: the Adapter is a separate
...
- **The `flue-contract` tier dissolves into the conformance suite.** Its rig — scripted OpenAI-compatible provider, real
  ...
  `test` gate: the opt-in-ness existed only because the tier owned a foreign pin and build. Its claims become j2
...
- **The pinning risk moves; it does not vanish.** pi releases ~2.4×/week and breaks across 0.x minors. The pin is exact,
  a bump is a deliberate kit change gated by the conformance suite, and the breaking surface is now a class j2
