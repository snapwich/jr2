...
jr2's product shape ([ADR-0009](0009-cli-and-instance-interface.md)) is a globally installed CLI and a standalone
Instance folder that just works — scaffold anywhere (`/tmp` included), install with whatever package manager the user
already has, `jr2 up`. None of that path existed: the scaffold wrote `workspace:*` deps only the kit's own pnpm
workspace can resolve, the bundle step was `pnpm deploy` (workspace members only), every package was `private: true`,
and installed mode's kit image refs name published tags that exist nowhere. Every test tier ran checkout mode, so the
mode users will actually run was the one mode nothing exercised. This ADR fixes the loop that closes that gap — and the
loop is **permanent, not a pre-1.0 crutch**: "test 1.3.0 before it exists on npm" is the same problem forever.

## The rule: fake the registry, never the mechanism

The loop feeds the real machinery a local upstream — the same philosophy as
[ADR-0038](0038-jr2-up-builds-every-image-it-deploys.md)'s scripted model provider (only the LLM is faked; here, only
the registries are local). Concretely: an ephemeral [verdaccio](https://verdaccio.org) npm registry per loop run;
`pnpm -r publish --registry` into it; `npm i -g @jr2/cli --registry` into a temp npm prefix; `jr2 init` in a temp dir
outside any checkout and any git repo; the user's own package manager installs; `jr2 up` converges in installed mode.
...
differently: npm cannot resolve inter-package deps (`@jr2/cli` → `@jr2/orchestrator`) to tarballs. A registry is the
only stand-in that exercises the real chain.
...
`node_modules`**, and an installed kit is nothing else — `jr2 --help` died on its own shebang. The image bundle already
knew (its entrypoint runs through `tsx`, ADR-0038); the host-side binary had nowhere to learn it.

So `bin/jr2.js` replaces `bin/jr2.ts`: it registers a `module.registerHooks` load hook that erases types with
...
may be written in, and the repo stays zero-build. **One entry for both worlds**, for the same reason `jr2 init` grows no
...
script refuses a `JR2_DIST_DIR` inside the checkout, and the `@dist` converge asserts the mode line it expects.

## The scaffold pins the kit that scaffolded it

`jr2 init` renders `"@jr2/orchestrator"` and `"@jr2/cli"` at the **exact** running `KIT_VERSION` — exact, not caret,
...
- **Standalone**: stage a copy of the instance (minus `node_modules/`, `.jr2/`, `.git/`, and the credential files
  `.env*`/`.npmrc` — `jr2 up` reads `.env` host-side into the Orchestrator's Secret, ADR-0019, so a copy would bake a
  key into an image layer and into the tag addressing it) and run a **frozen install from the lockfile**, dispatched on
...
(ADR-0008's instance repo, the planned `jr2 build`) runs from a clean checkout where **no `node_modules` exists**, a
...
the very thing that selects it (the user wrote the lockfile with it). jr2 itself depends on no package manager.
...
- **The manual loop**: a `just` recipe leaves verdaccio and the temp global install standing, so a developer plays the
  way a user lives — `jr2 init /tmp/anything && npm install && jr2 up`, entirely outside the repo.
...
- The npmjs `@jr2` scope must be claimed early regardless of readiness — the localhost guard protects against accidents,
  not squatters.
- **`jr2 up` still shells out to pnpm in one installed-mode shape**: an instance nested in the user's own pnpm
  workspace. That is not the product dependency creeping back, because the thing that selects pnpm is a file the user
  wrote with pnpm — the same self-selecting argument the lockfile dispatch already rests on.
