...
- **Harness container** — the CR's primary container: the Sandbox Image (user-built or user-brought,
  [ADR-0037](0037-an-instance-builds-its-sandbox-images-jr2-injects-the-harness.md)) with jr2's runtime mounted at
...
- **User Container** — opt-in third seat, composed only when the `workspace()` names its image (a `file:` docker context
  ...
  - **Unattended services.** A container's one command belongs to the Harness in the primary seat (ADR-0037), so an
    ...
    jr2 deliberately does not own. A system built _on_ jr2 that hands people access to their Sandboxes automates through
    this seat.
...
checkout per Repo the Sandbox names (ADR-0004, ADR-0051) — **and `/opt/jr2` read-only**, the runtime volume — so human
and agent see identical files. Neither `/repos` nor `/opt/jr2` is a second exception; each is half of the first. The
...
files jr2 does not own in this seat. Env-form config (`GIT_CONFIG_*` on the sidecar) was considered and rejected: it
would crack "jr2 puts nothing in it", and it does not even cover the seat's flagship access path — ssh login sessions
scrub container env — so the image would still owe a line for sshd while carrying jr2's env for exec sessions, two
...
jr2 sets `runAsUser` nowhere — each image's own `USER` decides its seat's uid — so the two writing seats may disagree,
and plain POSIX modes would then make the other seat's files read-only. jr2 closes that with two mechanisms it happens
to own, unconditionally (both are inert when the uids already match):
...
- **The pod carries `fsGroup` — the work group, default `2000`.** The default is convention; the one override lives on
  ...
  for the jr2 seats: Kubernetes grants the fsGroup as a supplemental group to every container process, so the Harness
...
- **The attach stamps a default ACL on every repo root it creates** — `u::rwx,g::rwx,o::r-x`, set by `work-acl`, a
  static helper on the runtime volume (part of ADR-0037's published `/opt/jr2` surface, static for the same reason `rg`
  is: it executes on a libc jr2 does not control). A default ACL is inherited by everything created beneath it, and
  POSIX ignores the process umask where one is present — so every file either seat writes under a repo tree lands
  group-writable (664/775), with **zero lines in any image**: no umask in a login shell, no jr2 knowledge in a brought
...
every tool child) and the attach script opens with the same line, so jr2's own writes are group-writable even there;
what a User Container session writes outside a repo tree is its own business.
...
- **A truly minimal Harness** (agent loop only, tools executed in a sibling container) — would need a cross-container
  execution transport jr2 does not have. Not blocking: the agent toolchain lives in the Sandbox Image; revisit only if
...
- **umask discipline as the sharing mechanism** (fsGroup + `umask 002` in every writer). Rejected as what the promise
  rests on: the Harness's half is one startup line jr2 controls, but the User Container's half is a line every image
  must remember (`[ -d /work ] && umask 002` in a login shell) — a gotcha, and a silent one: forgetting it costs nothing
...
- **A root init container stamping the ACL on `/work`'s own root** — complete coverage (no repo-tree scoping), owner
  ...
  `runAsNonRoot: false` container in _every_ pod, so no jr2 pod could satisfy the Pod Security "restricted" profile —
