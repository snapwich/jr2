...
[ADR-0038](0038-j2-up-builds-every-image-it-deploys.md) gave installed mode published-`<kitversion>` Kit image refs and
deferred the question of where those tags actually live "while nothing is published." The first installed-mode converge
against a real cluster ends the deferral: `publishedKitRefs()` emits bare `j2-harness:<kitversion>`, a node resolves
...
- **Kit images have a canonical public home: `ghcr.io/snapwich`.** `publishedKitRefs()` bakes it:
  `ghcr.io/snapwich/j2-harness:<kitversion>`, likewise adapter and operator. Public GHCR egress is free to the project,
  so an open-source kit can afford a home every user's nodes pull from — and only a canonical home makes
  `npm i -g @j2/cli && j2 init && j2 up` work with zero image plumbing, which is the product's promise. Pushing there is
  part of the release train ([ADR-0019](0019-one-converging-command-against-the-current-context.md)): a release is not
  done until the images for its npm version are at the home.
- **A new config key `kitRegistry` re-homes the Kit refs** — `<kitRegistry>/j2-harness:<kitversion>` — for self-hosted,
  ...
  → env.
- **Getting Kit images into a self-hosted registry is deliberate and instance-less — never part of `j2 up`.** Kit images
  are shared by every instance on a cluster (and possibly by every instance in an org); updating them is a deliberate
  act, not a side effect of converging one instance. Two arms, split by what each audience holds:
  - **`j2 kit push <registry>`** (in the binary, for the installed self-hoster): a **mirror**, nothing more —
...
- **No canonical home — self-host as the only model.** Rejected: every user's first `j2 up` fails until they stand up a
...
- **Mirroring inside `j2 up`** (absent tag → pull home, push self-host, converge on). Rejected: kit updates on a shared
...
- **One `j2 kit push` with a checkout build arm** (checkout-is-the-signal, like `j2 up`). Rejected: the build arm needs
  the whole repo, so it belongs to the repo's own tooling — ADR-0038 already scoped `just` recipes as "shortcuts for
  building images without a converge." The binary keeps only the arm its audience can use.
...
- **Accepted remainder: a re-pushed dev tag is stale on any node that pulled the old one.** The loop pushes
  `j2-harness:0.0.0` repeatedly; `IfNotPresent` keeps the first bytes a node saw. Version tags are immutable everywhere
...
- `just kit-push` needs a multi-platform buildx builder (qemu for the foreign arch). A checkout-mode converge against a
  foreign-arch cluster needs no workaround: `j2 up` derives the platform set from the cluster's nodes and passes
