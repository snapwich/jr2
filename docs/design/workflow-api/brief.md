# jr2 workflow-API redesign — grill outcomes (2026-07-13)

Goal: simplify the consumer-facing workflow API so `examples/coding/workflows/coding.ts` reads as pure workflow (states,
transitions, prompts, policy) with zero jr2-mechanism residue. Consumer implements their workflow; jr2 owns how it runs.
...
1. **Direction: mechanisms absorb plumbing, not templates.** ADR-0003's template/`provide()` model likely retires;
   ...
   introduced to hide jr2-internal complexity (e.g. workspace provisioning states) while a consumer machine runs
   "inside". Maybe another way accomplishes that. Multiple proposals / sub-proposals welcome.
2. **Offsets/durability plumbing → 100% jr2-internal.** Host persists iid→offset off the inspection stream (already
...
4. **Agent tools by convention.** Consumer specifies only prompt + genuinely custom tools. Workflow-driving tools
   (events) are inserted by jr2 — ideally DERIVED from the invoking state's event transitions (or demarcated as agent
...
5. **Retries absorbed.** jr2 internally retries infra faults and auto-nudges no-signal agents within a defaulted budget
   ...
   observable via jr2 visualize / run feed (jr2 telemetry, not machine events).
6. **Top-machine primitive: open, generalized.** A `pool`-like primitive for spawn-under-cap + completion collection is
   ...
   Source becomes one adapter over it. User open to jr2 owning the whole work loop if the API makes sense.
7. **Authoring surface: `jr2.setup` analog OK.** Statically imported, xstate-shaped, derives event unions from defs,
   pre-registers jr2 actors, injects mechanism event types. MUST return a plain xstate machine (Stately-inspectable,
   `.provide()`-testable). No jr2-only DSL. **The `export const events` manifest dies** — vocabulary derivable from the
   machine object.
...
- flue usage: study https://flueframework.com/ — is jr2 using flue (iids, endpoints, offsets, Harness) the way that
  makes most sense?
