...
plus compiled/executed experiments in `scratchpad/xstate-lab/` (`exp1-j2setup.ts`, `exp1b/1c`, `exp2-wrapper.ts`,
...
- Implementation needs exactly **one j2-internal cast** (re-asserting the merged `SetupReturn`); the consumer surface
  has none.

**Bonus surface for the same wrapper:** because j2Setup takes the event **defs as values** (not just types), it can
...
`meta: { j2: { deliver: "human" } }` on gate transitions is fully supported.
...
manifest is indeed derivable. Caveat: it contains event _names_ only; schemas must come from the defs (hence j2Setup
...
Alternative (also viable, fully static): j2Setup's `createMachine` can rewrite the config before handing it to xstate —
for each state that invokes `agentRun`, compute the derived tool set from the config itself and wrap the consumer's
`input` mapper to append it. Both approaches compose; the static one also feeds `j2 visualize`.
...
- The working pattern [verified, exp2 v2 compiles clean]: **loosely-typed implementation + precisely-declared public
  ...
  with one j2-internal `as` at the return. Consumers get: `spec`'s `input` fully typed; `InputFrom<typeof wrapped>` =
...
finals, `output: ({context}) => context.outcome`. j2Setup can make this the documented idiom (or ship a helper).
...
`spawnChild` from consumer code entirely via a pool primitive (§4) — then the hole lives in j2 code once.
...
- **`setup().createStateConfig`** (new in 5.32.x): typed reusable state fragments. j2 could ship state-config
...
- **Config rewriting inside j2Setup.createMachine** (machines are data): inject/augment states or inputs before xstate
...
coding.ts does: context counter + guard against cap + `spawnChild` + collect via done events. So a j2 `pool` would not
...
action leaves precisely the trace `j2 visualize` reads — and because the pool is j2-owned and written once, the "keep
...
So a host can observe **system-wide, with zero consumer bookkeeping**: iid→offset persistence (decision 2 — j2 already
rides this stream per GAP(5)), and retry telemetry (decision 5 — each internal re-invoke shows up as `@xstate.actor` +
events; a retry counter is derivable host-side, or j2's internals can `emit()` and the host `.on()`s refs captured from
`@xstate.actor`).
...
1. **Attach timing**: children of the _initial_ state are constructed during `createActor()` itself, not `.start()`
   ...
   creations (it still sees all subsequent events/snapshots). Hosts must pass `inspect` at `createActor` — j2's RunHost
...
3. **`emit()` from non-machine logic** (fromCallback/fromPromise scope `emit`) notifies only `.on()` listeners — it does
   ...
   `agentRun` should therefore ride either machine-level emits, the inspection-visible event flow, or j2's own host
   channel.
...
1. **Generic-over-body `setup()` doesn't infer** (exp2 v1). Reshapes decisions 1/6 implementation: every j2
...
2. **Inline (object-src) spawned children cannot be persisted** — runtime throw
   ...
   j2Setup's pre-registration is what makes decisions 2/6 durable. Invoked actors get auto-generated referenced srcs, so
   factory-internal `fromPromise`s are fine.
3. **Nested `on` accepts unknown event keys** (TS excess-property hole, exp1b — top-level `on` rejects them; values
   ...
   vocabulary. Reshapes decision 7: j2Setup must validate `machine.events` ⊆ (defs ∪ mechanism events) at
...
5. **Machine output only at root; done-event `output` is `unknown` in the root mapper.** Cast-free idiom exists
   (outcome-in-context); should be the documented j2 pattern, else every consumer final-state machine re-grows the
   coding.ts cast.
...
7. **Inspection misses initial-children creation if attached late; `emit` from callback logic is not in the inspection
   ...
   compatible with j2's current host-owned `createActor`.
