# Proposal: the simplified jr2 workflow API

Synthesis of the grill session (2026-07-13) + four research reports: `report-jr-parity.md`, `report-jr2-internals.md`,
`report-xstate.md`, and `report-flue.md`. Exhibit: `coding-rewrite.ts` (~200 lines vs 681; zero mechanism residue).

Two of those inputs were removed from this directory after their subject was superseded. `report-flue.md` studied how
jr2 used flue; [ADR-0027](../../adr/0027-the-harness-is-jr2s-own-server-flue-retires-the-wire-stays.md) retired flue, so
its findings no longer apply and ADR-0027 records the conclusion in more detail. `coding-rewrite.ts` drafted the
rewrite; the rewrite landed as `examples/coding/workflows/jr.ts`, which is the version to read.

## TL;DR

The consumer surface shrinks to **six statically-imported names**:

| Export                      | Consumer writes                                                 | jr2 owns                                                                                                                     |
| --------------------------- | --------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------- |
| `defineEvent`               | name, description, zod input, optional `audience` restriction   | validation, transport, delivery, tool/gate derivation                                                                        |
| `jr2Setup`                  | context/input/output types, event defs, own actors/guards       | mechanism event types in the union, pre-registered `agentRun`/`gate`, vocabulary attachment (manifest dies), typo validation |
...
Every piece was validated: **feasible against current jr2 internals** (most changes delete code; registration table, run
...
### 1.1 `jr2Setup` — the authoring surface (grill decision 7)

```ts
jr2Setup({ types, events: [defs], actors, actions, guards }).createMachine({...})
```

- Returns xstate's **public `SetupReturn`** type → `.createMachine()` yields a plain `StateMachine`:
  Stately-inspectable, `.provide()`-testable, no jr2 runtime needed to construct. Proven by compiled experiment
  (`xstate-lab/exp1-jr2setup.ts`): consumer surface has zero casts.
...
- Derivation happens **statically in `jr2Setup.createMachine`** (config rewrite appending the derived names to the
  invoke input) — names still ride serializable input, so the ADR-0007 restore path and invoke-time `resolveAccepts`
  validation are byte-for-byte unchanged. The same static walk feeds `jr2 visualize` ("this state's agent can call X,
  Y").
...
suited to this without inventing jr2-only config inside the transition table (no-DSL rule); audience is a property of
the event's contract (who may say this), matching the registration table's authorization dialects — and it keeps
ADR-0011's blessed handle-once-at-an-ancestor idiom, which pure structural (own-transitions-only) derivation would
break.
...
- **ADR-0013 security survives and strengthens**: `mayDeliverToAgent` compares the HMAC-verified token's pod name
  against the registration's sandbox — now written by jr2 from the same deterministic `workspaceName()` the token was
...
- **Session continuity — fresh by default, SETTLED (grill 2026-07-14)**: jr launches a _fresh session on every relaunch_
  ...
  travel across states with changing menus); jr2 fails loudly at invoke if a `continue` iid is already live (flue
...
- **Retries absorbed** (decision 5), two distinct fault classes, both invisible until terminal:
  - _Infra faults_ — delegate to flue's native `durability: { maxAttempts, timeoutMs }` (it already distinguishes
    completed/aborted/exhausted/timeout and never re-runs a completed tool); jr2 re-attaches from the offset ledger.
  - _No-signal_ (agent ended its turn without calling a menu tool — jr's dominant failure mode) — flue considers that a
    normal completed turn, so this stays a jr2-owned budgeted re-prompt ("you must call one of: …"). Flue's own `finish`
...
consumer-side casts all gone). Complementary tool for lighter cases: xstate 5.32's `setup().createStateConfig` lets jr2
...
- **Dissolves the visualize footgun by construction**: the one top-level `spawnChild` lives in jr2's pool code, once;
  the ecosystem has no competing primitive (verified vacuum — the canonical v5 pattern is exactly coding.ts's
  hand-rolled loop).
...
- **Terminal semantics are three-valued** (forced by the park-a-gate escalation decision): source drained + children
  ...
  Triage surfaces as run _status_ (feed / `jr2 runs`), not machine events; a `pool.deadlock` event can be added when a
  real workflow needs to react to it.

### 1.6 Cast-free idioms jr2 documents (or wraps)
...
5. **Terminal `agent.fault` covers "turn ended without an event"** — jr's dominant failure mode; the consumer triage
   hook (investigator) is a future state on the fault route, not jr2 policy.
...
- Inline (object-src) spawned children can't persist → everything durable is a registered string src — which `jr2Setup`
...
- Stacked in-flight features (B branches off unmerged A) under the PR flow — jr had merge-all; no jr2 story yet.
...
1. jr2Setup + defineEvent `audience` + vocabulary-on-machine (deletes manifest; enables the rest).
...
5. Tools/accepts derivation in jr2Setup.
