...
state machines, where each Agent runs in its own host-isolated Kubernetes pod (Sandbox) via the
[flue framework](https://flueframework.com/). Local development runs on [kind](https://kind.sigs.k8s.io/) (Kubernetes in
Docker).
...
| `harness/`      | TS   | The Sandbox image: a flue Harness hosting the Agents that run inside a Sandbox (server side). |
| `orchestrator/` | TS   | The Orchestrator pod image: xstate runtime + flue client + Actors, pieces, Machines (client). |
