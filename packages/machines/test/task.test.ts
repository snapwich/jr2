...
// scenario (ADR-0054's first Consequence): the only proof a shipped Machine works in j2 is the
...
test("as shipped, the walk reports both parts Open; the consumer's customize closes both", () => {
  ...
  // workspace() and j2's wrappers are transparent — so the fix line names no `actors` nesting.
...
test("working → finish parks at the review Gate, with the branch and worktree a human needs", async () => {
  ...
  assert.match(first.prompt ?? "", /Work in \/work\/target\/j2\/task-/);
...
test("a terminal agent.fault parks at the same Gate, and the NEXT Turn is a fresh conversation", async () => {
  ...
  // j2 has already retried, nudged and rerolled (ADR-0016/0027/0035); this is the one terminal
...
test("the door's Dials reach the Turn, and never touch the Agent's identity (ADR-0018)", async () => {
  ...
  // The DEFINITION is untouched: `j2 up` preflights the bound model, and the door overrides it for
