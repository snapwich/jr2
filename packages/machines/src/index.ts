// @j2/machines — the Machines the kit ships (ADR-0054). A fourth published package beside
// `@j2/{cli,orchestrator,agent-protocol}` (ADR-0043), on the same release train, exporting named
// Machines from this index: `import { task } from "@j2/machines"`.
//
...
// in maps `@j2/orchestrator` holds (vocabulary.ts, parts.ts) and in `xstate`'s own `provide` and
...
// cannot know the repository or pay for the model, so it says so rather than guessing. `j2 up`
