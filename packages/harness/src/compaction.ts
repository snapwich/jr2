...
// still sees, replaced by a summary plus a retained tail. jr2's shape is one long agentic turn per
...
//
// The thresholds are jr2-owned turn mechanics (ADR-0016): derived from the model, no author
// surface, not a Dial (ADR-0018). They are stated HERE — a jr2 file — rather than read from pi's
...
// Dials, and a summary jr2 no longer wants must not outlive the run that asked for it (ADR-0024).
 ...
 * everything before them becomes the summary. The value pi ships, restated as jr2's own. */
 ...
 * options itself. So ADR-0036's "fails after pi's retries" is jr2's to state, right here. */
const SUMMARY_RETRIES = 2;
const SUMMARY_RETRY_BASE_DELAY_MS = 500;

/**
 * The compaction settings for one model — the whole jr2-owned derivation, pure so the arithmetic is
...
/**
 * Whether a context of this size crosses the reserve — pi's own pure predicate on jr2's numbers.
 * ADR-0036 rejected waiting for pi to CALL `shouldCompact` (it has zero internal callers), not the
 * arithmetic; keeping the call at jr2's seat makes the conformance suite the canary if a pin moves
...
/** What the cut needs, gathered by the caller: the Session jr2 constructed (pi keeps its own
...
export async function compactIfOver(seat: CompactionSeat, messages: AgentMessage[]): Promise<Cut | undefined> {
  ...
  // `fromHook: false` — the summary is jr2's own computation, not a `session_before_compact` hook's
  ...
  // idle-only and manually invoked; jr2's mid-turn regime cuts repeatedly on one conversation, so it
