...
// user's, byte-for-byte, and j2's runtime arrives at pod time on the `/opt/j2` volume with the
...
//     (uid 1000, `HOME=/home/j2` on an emptyDir). The attach writes `$HOME/.gitconfig` and any real
...
/** Where the pod's `/opt/j2` volume puts j2's vendored binaries (`node`, `rg`). */
export const RUNTIME_BIN = "/opt/j2/bin";

/** The home the operator gives an image that declares no `USER` (ADR-0037). */
export const FALLBACK_HOME = "/home/j2";

/** Group-writable for the work group, world-unchanged (ADR-0005). */
export const WORK_UMASK = 0o002;

/**
 * The PATH a process gets when its image cleared the variable. Appending `/opt/j2/bin` to nothing
 * would leave a PATH holding j2's bin ALONE — the user's toolchain deleted, which is the exact
