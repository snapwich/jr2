...
//   - `resolveTarget` finds the orchestrator. `--url` / `J2_URL` (+ `J2_TOKEN`) short-circuits
...
//     `config.name` > folder name): the Instance token is read from the in-cluster `j2-instance`
//     Secret and the `j2-orchestrator` Service is port-forwarded for the life of the command —
...
/** Walk up from `cwd` to the directory containing `j2.config.ts`; `undefined` if there is none. */
...
export function resolveRoot(cwd: string): string {
  ...
  if (!root) throw new Error("not inside a j2 instance — no j2.config.ts found walking up from cwd");
  return root;
}
...
export type TargetOptions = {
  /** Explicit orchestrator address (`--url`); with `J2_URL` the escape hatch past the kube path. */
  url?: string;
...
export async function resolveTarget(io: Io, opts: TargetOptions): Promise<Target> {
  ...
  if (explicit) {
    ...
    return { url: explicit, token: io.env.J2_TOKEN };
  }
  ...
  if (!context) {
    throw new Error("no kube context — `kind create cluster` (or point kubectl at one), then `j2 up`");
  }
  activity(io, `→ context ${context} / namespace ${namespace}`);

  const ctx = opts.context ? { context: opts.context } : {};
  const token =
    io.env.J2_TOKEN ?? (await kube.readSecret({ namespace, name: INSTANCE_SECRET, key: "J2_INSTANCE_TOKEN", ...ctx }));
  if (!token) {
    throw new Error(
      `not deployed here — right context? (context ${context}, namespace ${namespace}; \`j2 up\` deploys)`,
    );
  }
