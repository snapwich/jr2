...
// folder's `tsconfig.json` — so `j2 up`'s answer and the editor's are one answer.
...
export const tscTypecheck: TypecheckPort = async (root) => {
  if (!existsSync(join(root, "tsconfig.json"))) {
    throw new Error(
      "this instance has no tsconfig.json — `j2 init` scaffolds one that extends " +
        "`@j2/orchestrator/tsconfig.instance.json`, and `j2 up` typechecks the folder before it builds anything",
    );
  }
...
function compilerBin(root: string): string {
  ...
  const requireFrom = createRequire(join(root, "j2.config.ts"));
  let manifest: string;
  try {
    ...
    throw new Error(
      "this instance's TypeScript compiler is missing — install its dependencies (npm, pnpm, or bun), then " +
        "re-run `j2 up`. The scaffold pins `typescript` in devDependencies because the instance's program " +
