...
#
# Nothing here is a `jr2 up`, and nothing here is per-instance (ADR-0044): Kit images are shared by
# every instance on a cluster, so putting them somewhere is a deliberate act with no instance in it.
set -euo pipefail

registry="${1:-}"
# The platforms the kit RELEASES for — both of the architectures a jr2 cluster runs on today (the home
# cluster's nodes are arm64, this checkout's dev box is amd64); see docs/adr/0044, docs/adr/0045.
#
# MIRRORS `SUPPORTED_PLATFORMS` in packages/cli/src/build.ts, the set a checkout `jr2 up` intersects
...
# One release train (ADR-0019): the image tag IS the npm version. `@jr2/cli`'s manifest, because the
