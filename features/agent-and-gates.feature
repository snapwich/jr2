Feature: Agents and gates drive a run from outside
  ...
  `j2 dev` stub Harness and parks; the run is then driven entirely from outside — the agent
  played over its MCP surface (`/mcp/<iid>`), the human played over the gates API. Each event
  lands on the state that invoked its actor; leaving the state destroys the surface.
  ...
  Rule: an agent's tool call transitions the state that invoked it
    ...
    Scenario: the agent surface serves exactly the invoking state's tools
      ...
      Then the agent's MCP surface offers exactly "request_review"
    ...
    Scenario: an agent tool call moves the run
      ...
      And the agent's MCP surface is gone
