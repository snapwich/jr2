...
// `j2 dev` server, black-box like everything else here.
...
Then("stdout is a visualizer url for {string}", function (this: E2EWorld, wf: string): void {
  ...
  assert.equal(url, `${this.dev?.url}/viz/${wf}`, "the page rides on the serving orchestrator");
});
...
Then("the Machine structure is served for {string}", async function (this: E2EWorld, wf: string): Promise<void> {
  const res = await fetch(`${this.dev?.url}/workflows/${wf}/machine`);
  assert.equal(res.status, 200);
