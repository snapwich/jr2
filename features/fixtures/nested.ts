  ...
  states: {
    working: {
      invoke: {
        ...
        // there is no enclosing `workspace()` for the Agent to resolve one from (ADR-0016).
        input: ({ context }) => ({ prompt: "Do the work.", endpoint: context.endpoint }),
      },
    },
  },
});
...
export const machine = jr2Setup({
  ...
  id: "nested",
  context: ({ input }) => ({ endpoint: input.endpoint }),
  // Parallel, so both conversations are live at once: each child mints its own instance id from
  // its own actor path (ADR-0016), so the two `coder`s never collide on one address.
  type: "parallel",
  states: {
